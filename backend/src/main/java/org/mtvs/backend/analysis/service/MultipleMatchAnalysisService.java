...
public class MultipleMatchAnalysisService {
     ...
     * 다중 매치 분석 초기 레코드 생성
     */
    public MultipleMatchAnalysis createInitialRecord(AccountDto account, int matchCount) {
        logger.info("Creating initial multiple match analysis record for puuid: {} with matchCount: {}", account.getPuuid(), matchCount);
        ...
        analysis.setAnalysisStatus(MultipleMatchAnalysis.AnalysisStatus.REQUESTED);
        
        return multipleMatchAnalysisRepository.save(analysis);
    }

    /**
     * 매치 ID 목록으로 레코드 업데이트
     */
    public MultipleMatchAnalysis updateWithMatchIds(String puuid, int matchCount) {
        logger.info("Updating multiple match analysis with matchCount: {} for puuid: {}", matchCount, puuid);
        
        // 가장 최근 REQUESTED 상태의 레코드 찾기
        List<MultipleMatchAnalysis> requestedAnalyses = multipleMatchAnalysisRepository
                .findByAnalysisStatusOrderByCreatedAtDesc(MultipleMatchAnalysis.AnalysisStatus.REQUESTED);
        
        MultipleMatchAnalysis analysis = requestedAnalyses.stream()
        ...
        try {
                ...
                try {
                    ...
                    if (firstMatch != null && firstMatch.getInfo() != null) {
                        String playerName = firstMatch.getInfo().getParticipants().stream()
                                ...
                                .orElse(analysis.getTargetPlayerName());
            ...
            logger.error("Failed to update match IDs for puuid: {}", puuid, e);
            throw new IllegalArgumentException("매치 목록 업데이트 실패: " + e.getMessage());
        }
    }

    /**
     * 기존 레코드에서 다중 매치 AI 분석 수행
     */
    public Map<String, Object> performMultipleAIAnalysisFromExistingRecord(String puuid) {
        logger.info("Performing multiple AI analysis from existing record for puuid: {}", puuid);
        ...
        try {
            ...
            multipleMatchAnalysisRepository.save(analysis);
            
            List<String> selectedMatchIds = analysis.getAnalyzedMatchIds();
            if (selectedMatchIds == null || selectedMatchIds.isEmpty()) {
                throw new IllegalArgumentException("분석할 매치 목록이 설정되지 않았습니다. 2단계를 먼저 수행해주세요.");
            }
            
            // 매치 데이터 수집
            List<MatchDetailDto> matchDetails = new ArrayList<>();
            List<MatchTimelineDto> matchTimelines = new ArrayList<>();
            List<String> successfulMatchIds = new ArrayList<>();
            ...
            // AI 분석 수행
            Map<String, Object> aiRequest = buildAIRequestData(matchDetails, matchTimelines, puuid, analysis.getMatchCount(), successfulMatchIds);
            ...
            String aiResponse = gameAnalysisService.analyzePlayerMultipleMatches(gameName, tagLine, analysis.getMatchCount());
            ...
            Map<String, Object> aiResponseData = Map.of(
                    ...
                    "matchCount", analysis.getMatchCount(),
                    "analyzedMatchIds", selectedMatchIds
            );
            
            analysis.setAiResponseData(aiResponseData);
            analysis.setAnalysisSummary(aiResponse);
            analysis.setAnalysisStatus(MultipleMatchAnalysis.AnalysisStatus.COMPLETED);
            
            MultipleMatchAnalysis finalAnalysis = multipleMatchAnalysisRepository.save(analysis);
            
            return buildResponseData(finalAnalysis);
            
        } catch (Exception e) {
            logger.error("Multiple AI analysis failed for puuid: {}", puuid, e);
            analysis.setAnalysisStatus(MultipleMatchAnalysis.AnalysisStatus.FAILED);
