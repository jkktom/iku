...
//        prompt.append("   - 포지셔닝 및 맵 운영 개선 방안 (포지션별 특화)\n\n");
        ...
        prompt.append("=== 최종 출력 형식 ===\n");
        prompt.append("분석이 끝나면, 최종 결과를 다음 두 가지 형식으로 나누어 제공해주세요:\n");
        prompt.append("1. **JSON 데이터**: 분석 내용을 정해진 JSON 구조에 맞춰 정리해주세요. 이 데이터는 시스템에서 직접 사용됩니다.\n");
        prompt.append("2. **친근한 피드백 (텍스트)**: 위 JSON 데이터를 바탕으로, 플레이어에게 직접 코칭하듯이 친근하고 이해하기 쉬운 말투로 피드백을 작성해주세요. 이 피드백은 '원본 AI 리포트 보기' 기능에 사용됩니다. 칭찬과 격려를 섞어 동기를 부여하는 톤을 유지해주세요.\n");
